...
function FileDownload (props) {
  ...
  useEffect(() => {
    if (downloadURL) {
      pdcAxiosInstance.head(downloadURL)
        .then(({ headers }) => {
          ...
          linkText={(
            <>
              ...
              {isDatasetExplorerDownloadLink ? (
                <span>Download full dataset ({format.toUpperCase()}) {`${downloadSize.size} ${downloadSize.sizeLabel}`}</span>
              ) : (
                <span>Download full dataset <span className='sr-only'>- {format} - {`${downloadSize.size} ${downloadSize.sizeLabel}`}</span></span>
              )}
            </>
          )}
