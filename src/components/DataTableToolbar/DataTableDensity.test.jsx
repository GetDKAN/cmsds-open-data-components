...
const componentArgs = {
  ...
  items: [
    {
      icon: <FontAwesomePro icon="density-1" aria-hidden />,
      text: 'Expanded',
      value: 'density-1',
    },
    {
      icon: <FontAwesomePro icon="density-2" aria-hidden />,
      text: 'Normal',
      value: 'density-2',
    },
    {
      icon: <FontAwesomePro icon="density-3" aria-hidden />,
      text: 'Compact',
      value: 'density-3',
    },
  ],
...
describe('DataTableDensity component.', () => {
  it('Matches snapshot.', () => {
    const renderedDataTableDensity = renderer
      .create(<DataTableDensity {...componentArgs} />)
      .toJSON();
  ...
  it("'densityChange' prop is called when buttons are clicked.", () => {
    ...
    const { container } = render(
      <DataTableDensity {...componentArgs} densityChange={densityChange} />
    );

    componentArgs.items.forEach((button) => {
      fireEvent.click(
        container.querySelector(`button[aria-label*="${componentArgs.title}, ${button.text}"]`)
      );
    ...
    componentArgs.items.forEach((button) => {
      expect(
        container.querySelector(
          `button[aria-label*="${componentArgs.title}, ${button.text}"] > span`
        )
