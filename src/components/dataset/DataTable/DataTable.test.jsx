...
describe('DataTable component.', () => {
  ...
  it("The 'Next' and 'Previous' buttons change data pages in the table.", async () => {
    ...
    expect(element.container.querySelector('.-pageInfo')).toHaveTextContent(
      'Page 2 of 2 for Home Health Care - Measure Date Range'
    );

    await waitFor(() =>
      userEvent.click(screen.getByRole('button', { name: 'Previous data page' }))
    );
    expect(element.container.querySelector('.-pageInfo')).toHaveTextContent(
  ...
  it('User can resize the table columns with a keyboard', async () => {
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'Measure Name has been resized. The new width is 160 pixels.'
    );
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'Measure Name has been resized. The new width is 150 pixels.'
    );
    ...
    expect(
      element.container
        .querySelector('[aria-label="Resize Measure Name column"]')
        .classList.contains('isResizing')
    ).toBe(false);
  ...
  it("Columns don't resize above 400px", async () => {
    ...
    // ArrowRight key to increase the first column width 10px
    for (let i = 1; i <= 26; i++) {
      // Iterate 26 times to set the width to 410px (Increments by 10 and starts at 150)
      fireEvent.keyDown(screen.getByLabelText('Resize Measure Name column'), {
        key: 'ArrowRight',
        code: 'ArrowRight',
        keyCode: 39,
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'Measure Name has been resized. The new width is 400 pixels.'
    );
  });
  ...
  it("Columns don't resize below 30px", async () => {
    ...
    // ArrowRight key to increase the first column width 10px
    for (let i = 1; i <= 13; i++) {
      // Iterate 13 times to set the width to 20px (Increments by 10 and starts at 150)
      fireEvent.keyDown(screen.getByLabelText('Resize Measure Name column'), {
        key: 'ArrowLeft',
        code: 'ArrowLeft',
        keyCode: 37,
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'Measure Name has been resized. The new width is 30 pixels.'
    );
  });
  ...
  it('User can sort the table columns', async () => {
    ...
    expect(
      element.container
        .querySelector('.dc-thead > .tr > .th:first-child .sort-icon > button')
        .getAttribute('aria-label')
    ).toEqual('Sort Measure Name column in ascending order');

    // Check sorting asc
    await act(async () => {
      fireEvent.click(
        element.container.querySelector('.dc-thead > .tr > .th:first-child .sort-icon > button')
      );
    });
    expect(
      element.container
        .querySelector('.dc-thead > .tr > .th:first-child .sort-icon > button')
        .getAttribute('aria-label')
    ).toEqual('Sort Measure Name column in descending order');
    ...
    expect(
      element.container
        .querySelector('.dc-thead > .tr > .th:first-child .sort-icon > button')
        .getAttribute('aria-label')
    ).toEqual('Reset Measure Name column sort order');
