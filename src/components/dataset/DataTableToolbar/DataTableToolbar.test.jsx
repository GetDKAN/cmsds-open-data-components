...
const mockApiResponse = {
  ...
  query: {
    ...
    format: 'json',
    rowIds: false,
    properties: ['measure_name', 'measure_date_range'],
  },
};
...
const renderComponent = async (args = componentArgs) =>
  render(
...
describe('DataHeaderButton component.', () => {
  ...
  it('Matches snapshot.', async () => {
    let renderedDataTableToolbar;

    await act(async () => {
      renderedDataTableToolbar = await renderComponent();
    });

    expect(renderedDataTableToolbar.asFragment()).toMatchSnapshot();
  });
  ...
  it('User can set a filter and remove a filter for a dataset.', async () => {
    ...
    fireEvent.change(screen.getAllByLabelText('Select column')[0], {
      target: { value: 'measure_name' },
    });

    // Choose a condition for first filter
    fireEvent.change(screen.getAllByLabelText('Select condition')[0], {
      target: { value: 'LIKE' },
    });
    ...
    fireEvent.change(screen.getAllByLabelText('Select column')[1], {
      target: { value: 'measure_date_range' },
    });

    // Choose a condition for second filter
    fireEvent.change(screen.getAllByLabelText('Select condition')[1], {
      target: { value: 'LIKE' },
    });

    // Type a value for second filter
    fireEvent.change(screen.getAllByLabelText('Enter value')[1], {
      target: { value: 'January 1, 2018 - December 31, 2018' },
    });
    ...
    expect(element.container.querySelector('.data-table-results > p')).toHaveTextContent(
      'Viewing 1 - 2 of 2 rows'
    );
    expect(
      screen.queryByRole('button', { name: 'Remove measure_name contains Average filter' })
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('button', {
        name: 'Remove measure_date_range contains January 1, 2018 - December 31, 2018 filter',
      })
    ).toBeInTheDocument();
  ...
  it('User can reorder the columns for a dataset.', async () => {
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'Measure Name grabbed'
    );
    ...
    expect(element.container.querySelector('.aria-live-feedback')).toHaveTextContent(
      'The rankings have been updated, Measure Name is now ranked 2 of 2'
    );
  ...
  it('User can change the row height of the data table.', async () => {
    ...
    expect(screen.queryByRole('button', { name: 'Row height, Expanded - selected' })).toHaveClass(
      'active'
    );

    // Click 'Normal' option
    fireEvent.click(screen.getByRole('button', { name: 'Row height, Normal' }));
    expect(screen.queryByRole('button', { name: 'Row height, Normal - selected' })).toHaveClass(
      'active'
    );

    // Click 'Compact' option
    fireEvent.click(screen.getByRole('button', { name: 'Row height, Compact' }));
    expect(screen.queryByRole('button', { name: 'Row height, Compact - selected' })).toHaveClass(
      'active'
    );
    ...
    expect(
      element.container.querySelector('.popover-row.data-table-density')
    ).not.toBeInTheDocument();
  });
